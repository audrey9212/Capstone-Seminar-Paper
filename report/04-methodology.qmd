...
#### MLP trunk.
The concatenated representation (embeddings + continuous + binary) is passed through a feedforward network with ReLU activations and dropout regularization between layers. A typical configuration uses two hidden layers with decreasing width (e.g., 128 → 64), though the exact architecture can be varied experimentally.
...
Four blending strategies were evaluated:

1. **Simple Average**: Equal-weight averaging of predicted probabilities: $\hat{p}_{\text{blend}} = \frac{1}{K} \sum_{k=1}^{K} \hat{p}_k$

2. **AUC-Weighted Average**: Weights proportional to excess validation ROC-AUC above the random baseline ($0.5$): $w_k \propto \max(\text{AUC}_k - 0.5,\; 0)$. This assigns zero weight to models that perform no better than chance and allows strong models to dominate.
...
candidates = []
if fig_dir.exists():
    for pat in ["stacking_schematic*.png", "07_stacking*.png", "*stacking*arch*.png", "*oof*stacking*.png"]:
        candidates.extend(sorted(fig_dir.glob(pat)))
...
At inference time, all base models are applied to the test set to produce a prediction matrix $Z_{\text{test}}$. The meta-learner then combines these predictions to produce final ensemble probabilities. The operating threshold ($\tau = 0.44$) is applied to these ensemble probabilities to generate binary churn predictions for evaluation.
