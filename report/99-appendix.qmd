...
Two OOF stacking variants were trained. The four-model variant (Stacking_OOF) used out-of-fold predictions from Logistic Regression, XGBoost, LightGBM, and Random Forest as meta-features for a logistic regression meta-learner with L2 regularization. The five-model variant (Stacking_OOF_5) added Wide & Deep (Focal) as a fifth base learner, whose OOF predictions were generated via 5-fold cross-validation to prevent leakage. XGBoost and LightGBM received the largest meta-learner coefficients in both variants, consistent with their superior individual ranking performance; Wide & Deep contributed complementary signal that lifted stacking performance from test ROC-AUC 0.6679 to 0.6702. Exact meta-learner coefficients were not serialized separately but can be reproduced by re-running `scripts/07_stacking.py`.
