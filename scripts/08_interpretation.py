...
def generate_plots(
    ...
    for name, info in sorted(roc_data.items(), key=lambda x: x[1].get("auc", 0), reverse=True):
        proba = info["proba"]
        fpr, tpr, _ = roc_curve(y_test, proba)
        roc_auc = info.get("auc", roc_auc_score(y_test, proba))
        ax_roc.plot(fpr, tpr, label=f"{name} (AUC={roc_auc:.4f})", linewidth=2)
    ...
    ax_roc.set_title("ROC Curve Comparison - Test Set")
    ax_roc.legend(loc="lower right", fontsize=9)
    ax_roc.grid(alpha=0.3)

    baseline_precision = y_test.mean()
    for name, info in sorted(pr_data.items(), key=lambda x: x[1].get("pr_auc", 0), reverse=True):
        proba = info["proba"]
        precision, recall, _ = precision_recall_curve(y_test, proba)
        pr_auc = info.get("pr_auc", average_precision_score(y_test, proba))
        ax_pr.plot(recall, precision, label=f"{name} (PR-AUC={pr_auc:.4f})", linewidth=2)

    ax_pr.axhline(baseline_precision, color="k", linestyle="--", linewidth=1,
                  label=f"Baseline ({baseline_precision:.4f})")
    ax_pr.set_xlim([0.0, 1.0])
    ax_pr.set_ylim([0.0, 1.05])
    ax_pr.set_xlabel("Recall")
    ax_pr.set_ylabel("Precision")
    ax_pr.set_title("Precision-Recall Curve Comparison - Test Set")
    ax_pr.legend(loc="lower left", fontsize=9)
    ax_pr.grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig(output_dir / "08_model_comparison2.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"    [OK] 08_model_comparison2.png")
...
def main():
    ...
    print("  - artifacts/figures/08_model_comparison2.png")
