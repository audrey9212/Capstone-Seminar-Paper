...
This thesis presented a comprehensive exploration of telecom customer churn prediction, balancing **predictive performance** with **interpretability** and practical actionability. A range of modeling approaches was investigated under consistent conditions, class imbalance challenges were tackled, and insights were extracted to guide retention strategy. The findings are summarized below in relation to the three research questions posed in the Introduction (Chapter 1).

### RQ1: Model Effectiveness {#sec-rq1-answer}

**Results indicate** that gradient-boosted decision trees achieved the strongest overall performance among all model families tested. Among single models, selection was based on validation ROC-AUC per the protocol in @sec-eval-protocol: XGBoost ranked first on validation (val ROC-AUC = 0.66) and is therefore the **champion model by selection criterion**, achieving a test ROC-AUC of **0.66** and a test PR-AUC of **0.45**. Notably, LightGBM achieved marginally higher test metrics (test ROC-AUC = **0.67**, test PR-AUC = **0.45**), demonstrating that the two GBM approaches are effectively matched; the difference in validation-set rank reflects sample variability rather than a fundamental performance gap. Both substantially outperformed the logistic regression baseline (test ROC-AUC = 0.59, test PR-AUC = 0.36). The best heterogeneous ensemble (NNLS-optimized blending) achieved a higher test ROC-AUC of **0.67** and test PR-AUC of **0.45**, but the marginal gain over the best single models is modest.

Heterogeneous ensemble methods (blending and OOF stacking) provided competitive but not decisively superior results. The best blending approach (NNLS-optimized) reached a test ROC-AUC of **0.67** and test PR-AUC of **0.45**. A five-model OOF stacking variant incorporating Wide & Deep as a fifth base learner (Stacking_OOF_5) achieved test ROC-AUC of **0.67** and test PR-AUC of **0.45**, nearly matching the best blend and substantially improving over the four-model OOF stacking (test ROC-AUC = 0.67). Ensemble methods slightly exceeded XGBoost on ROC-AUC, but the improvement was modest. This suggests that the gains from model combination derive primarily from variance reduction and diversity between linear, nonlinear, and neural base learners, rather than capturing fundamentally new predictive signals.

The **Wide & Deep neural network**, a model architecture not commonly used in traditional churn studies, proved feasible on tabular telecom data. The best neural network variant (Wide & Deep with focal loss) achieved test ROC-AUC of **0.66** and test PR-AUC of **0.44**, demonstrating that deep learning approaches can capture churn patterns effectively, though they did not surpass gradient boosting methods in this dataset. The neural networks clustered in a narrow performance band (test ROC-AUC approximately 0.64–0.66), suggesting a performance ceiling for this architecture class on this feature set.
...
- Adjusting the **decision threshold** on the validation set to optimize recall/precision trade-offs rather than using the default 0.50

The findings indicate that these techniques did improve the model's ability to capture churners. At the fixed operating threshold of τ = 0.44, LightGBM achieved the best balance with **precision = 0.38, recall = 0.73, and F1 = 0.50**. XGBoost performed nearly identically with precision = 0.37, recall = 0.74, and F1 = 0.49. This balance is **advantageous for retention contexts**, facilitating the detection of approximately 73–74% of actual churners while maintaining precision in the high-30s%, a trade-off suitable for targeted retention campaigns where the cost of contacting non-churners is acceptable.

Experiments with **focal loss** produced competitive ranking metrics (ROC-AUC, PR-AUC) but exhibited threshold sensitivity: under the fixed threshold of τ = 0.44, some focal-loss variants produced very few positive predictions, resulting in near-zero recall despite competitive AUC scores. This highlights that **loss function selection must be paired with appropriate threshold calibration** for deployment. The standard weighted cross-entropy loss proved more robust across operating points in this dataset.
...
In evaluating the above points, it is worth noting an **engineering trade-off** that emerged. While ensemble methods achieved comparable performance to single models, they introduce significant operational complexity without proportionate performance gains. The marginal improvements (approximately 0.01–0.01 in ROC-AUC) must be weighed against deployment and maintenance costs.

| Consideration | Ensemble (Blending/Stacking) | LightGBM | XGBoost |
|---------------|------------------------------|----------|---------|
| Test ROC-AUC | 0.67–0.67 | 0.67 | 0.66 |
| Test PR-AUC | 0.45–0.45 | 0.45 | 0.45 |
| Val Selection Rank | — | 2nd (val = 0.65) | 1st (val = 0.66) |
| Inference Latency | Higher (multiple models) | Low | Low |
| Deployment Complexity | High | Low | Low |
| Maintenance Burden | Multiple model artifacts | Single artifact | Single artifact |
| Interpretability | Requires aggregation | Direct SHAP analysis | Direct SHAP analysis |
| Threshold Calibration | More complex | Straightforward | Straightforward |

: Comparison of deployment considerations across ensemble methods and the two leading single GBM models. {#tbl-deployment-tradeoffs}

For a **real-time deployment** where latency and system simplicity are paramount, either LightGBM or XGBoost is a sound choice. LightGBM achieved the highest test-set performance among all single models (test ROC-AUC = 0.67, test PR-AUC = 0.45) and is the recommended production choice on the basis of held-out evaluation. XGBoost ranked first on the validation set under the pre-specified selection criterion (val ROC-AUC = 0.66), and its test-set difference from LightGBM (0.01 ROC-AUC) is small enough that either model is viable depending on infrastructure preferences. The ensemble approach might be justified in batch-scoring scenarios where the slight PR-AUC lift translates to meaningful business value at scale, but the complexity trade-off generally favors single-model deployment.

This highlights that **model selection must consider deployment constraints, maintainability, and interpretability requirements** alongside raw performance, and should distinguish between validation-based selection (favors XGBoost) and held-out test evaluation (favors LightGBM). The practical recommendation is to deploy LightGBM for production while treating XGBoost as a validated fallback — both models deliver strong, interpretable churn predictions with low operational overhead.
...
One promising direction is to leverage unlabeled or partially labeled data through **semi-supervised techniques**. The current study attempted pseudo-labeling using the XGBoost teacher model on real holdout data, but achieved minimal coverage: at conservative confidence thresholds (0.05/0.95), only 2 samples out of 20,000 met the criteria, with approximately 100% remaining "uncertain." This result highlights that **confidence calibration and threshold selection are critical** for successful semi-supervised learning.
...
The autoencoder experiments in this thesis (see @sec-autoencoder-interpretation) showed that learned latent representations did not significantly improve downstream classification beyond the original feature set (augmented features achieved ROC-AUC = 0.65 vs. baseline 0.65). However, the autoencoder architecture remains valuable as an **engineering capability**: it provides a reusable representation learning component and a clear interface for future semi-supervised variants if better confidence calibration or more unlabeled data becomes available.
:::
...
In conclusion, the journey of building this churn prediction system has shown both the substantial value such models can deliver and the complexity of making them truly effective in practice. The key findings, indicating that gradient-boosted trees (with XGBoost achieving test ROC-AUC = 0.66) outperform other model families, that the best ensemble (NNLS-optimized blending with five base learners) offers only a modest lift to test ROC-AUC = 0.67, that adding Wide & Deep as a fifth stacking base learner improves OOF stacking from 0.67 to 0.67, that class imbalance handling requires both algorithmic strategies and threshold tuning, and that SHAP-based interpretability enables actionable business insights, provide a solid foundation for production deployment.
