...
In conclusion, the journey of building this churn prediction system has shown both the substantial value such models can deliver and the complexity of making them truly effective in practice. The key findings, indicating that gradient-boosted trees (with XGBoost achieving test ROC-AUC = 0.6637) outperform other model families, that the best ensemble (NNLS-optimized blending with five base learners) offers only a modest lift to test ROC-AUC = 0.6704, that adding Wide & Deep as a fifth stacking base learner improves OOF stacking from 0.6679 to 0.6702, that class imbalance handling requires both algorithmic strategies and threshold tuning, and that SHAP-based interpretability enables actionable business insights, provide a solid foundation for production deployment.
